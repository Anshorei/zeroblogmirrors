use std::ffi::OsStr;
use std::{path::PathBuf, sync::mpsc::Receiver};
use notify::{DebouncedEvent, INotifyWatcher, Watcher, RecursiveMode};
use log::*;
use crate::shared_state::SharedState;
use std::sync::{Arc, Mutex};
use std::fs::{self, File};
use crate::args::Args;

pub fn watch(args: Args, mut watcher: INotifyWatcher, receiver: Receiver<DebouncedEvent>, shared_state: Arc<Mutex<SharedState>>) {
  info!("Listening for file changes...");
  let mut file_watcher = FileWatcher {
    notify_watcher: watcher,
    shared_state,
    args,
  };

  file_watcher.load_content();
  file_watcher.load_data();

  loop {
    match receiver.recv() {
      Ok(DebouncedEvent::Remove(path)) | Ok(DebouncedEvent::Write(path)) => file_watcher.load_file(path),
      Ok(event) => info!("{:?}", event),
      Err(e) => error!("{:?}", e),
    }
  }
}

struct FileWatcher {
  notify_watcher: INotifyWatcher,
  shared_state: Arc<Mutex<SharedState>>,
  args: Args,
}

impl FileWatcher {
  fn load_file(&mut self, path: PathBuf) {
    match path.file_name() == Some(OsStr::new("content.json")) {
      true => self.load_content(),
      false => self.load_data(),
    }
  }

  fn load_content(&mut self) {
    let path = format!("{}/{}/content.json", self.args.zeronet_path, self.args.site_address);
    info!("Reading from {}", path);
    let contents = fs::read_to_string(path.clone()).unwrap();

    let mut state = self.shared_state.lock().unwrap();
    state.content = serde_json::from_str(&contents).unwrap();

    info!("Watching {}", path);
    self.notify_watcher.watch(path, RecursiveMode::NonRecursive);
  }

  fn load_data(&mut self) {
    let path = format!("{}/{}/data/data.json", self.args.zeronet_path, self.args.site_address);
    info!("Reading from {}", path);
    let contents = fs::read_to_string(path.clone()).unwrap();

    let mut state = self.shared_state.lock().unwrap();
    state.data = serde_json::from_str(&contents).unwrap();

    info!("Watching {}", path);
    self.notify_watcher.watch(path, RecursiveMode::NonRecursive);
  }
}

fn update_state(watcher: &mut INotifyWatcher, shared_state: Arc<Mutex<SharedState>>, path: PathBuf) {
  let contents = fs::read_to_string(path.clone()).unwrap();

  watcher.watch(path, RecursiveMode::NonRecursive);

  let state = shared_state.lock().unwrap();
  debug!("Content: {:?}", state.content.modified);
  debug!("Data: {:?}", state.data.modified);
}
